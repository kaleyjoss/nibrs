...
replace months_of_data_each_year = 0 if missing(months_of_data_each_year)
replace months_of_data_total = 0 if missing(months_of_data_total)

// Filter to rows of 100%-missing agencies
...
local num_agencies_with_zero_data : word count `agencies_with_zero_data'
di `num_agencies_with_zero_data'

// Count number of agencies, zero-data-agencies for each county
bysort fips_state_county_code_crosswalk ori: gen _tag = _n == 1
bysort fips_state_county_code_crosswalk: egen num_agencies = total(_tag)
drop _tag

bysort fips_state_county_code_crosswalk ori: gen _tag = _n == 1 & months_of_data_total == 0
bysort fips_state_county_code_crosswalk (months_of_data_total): egen num_agencies_with_zero_data = total(_tag)
drop _tag


gen has_any_data = (months_of_data_total!=0)
...
keep ori crosswalk_agency_name fips_state_county_code_crosswalk county_population crosswalk_population year num_agencies num_agencies_with_zero_data months_of_data_each_year population_any_data population_x_num_months population_x_total_months
duplicates drop
save "$clean/missingness_by_population_agency.dta", replace
...
keep fips_state_county_code_crosswalk crosswalk_population year c_yr_pop_covered_any perc_c_yr_pop_covered_any c_pop_covered_any perc_c_pop_covered_any
duplicates drop 
gen fips_5_digit = string(fips_state_county_code_crosswalk, "%05.0f")
save "$clean/missingness_by_county.dta", replace
