...
*-----------------------------*
* 3. Crosswalk agencies from ICPSR_35158
*-----------------------------*

use "$raw/ICPSR_35158/DS0001/35158-0001-Data.dta", clear
rename ORI9 ori9
replace ori9 = ORI7 + "00" if missing(ori9)
keep if ori9 != "-1"
drop if missing(ori9)
...
merge 1:m year month ori9 using `combined_data'
sort ori9 year month


*-----------------------------*
* 7. Check that each agency/year only has 12 or less rows-- that the merging in didn't add any rows/duplicate months
*-----------------------------*
bysort ori9 year : gen num_rows = _N
tab num_rows

*-----------------------------*
* 8. Out of 2.5million rows, there are 1294 with num_months>12 (99.43% have correct number). However, if I ignore the below indicator variables, and then drop duplicates, those rows are duplicates, and there's only 12 per agency/year. So these must have been double imports. So i'm ignoring/making missing these. And now its only 12/agency/year 
*-----------------------------*
...
*-----------------------------*
* 9. Out of 36,490 ORI/agencies in the 2012 data, there are 647 missing NIBRS crosswalk.  
*-----------------------------*
gen missing_nibrs_crosswalk = missing(FSTATE)
bysort ori9: gen _tag = _n == 1 
levelsof ori9 if _tag==1 & missing_nibrs_crosswalk==1, local(agencies_missing_crosswalk)
drop _tag
local num_agencies_missing_crosswalk : word count `agencies_missing_crosswalk'
di `num_agencies_missing_crosswalk'
